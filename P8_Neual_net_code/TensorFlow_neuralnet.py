...
thumbnail_dir = 'training_data_NEWEST/'  # Path to the directory containing thumbnail images
#thumbnail_dir = 'training_data_backup/'
model_filename = 'tensorflow_simpleneuralnet_model.keras' # name of file containing trained model
...
def model():
    model = Sequential([
        layers.Flatten(input_shape=(17,17)),
        layers.Rescaling(1./255, input_shape=(1, 289)),
        #layers.Dense(16, activation=tf.nn.relu),
        #layers.Dropout(0.5),
        layers.Dense(64, kernel_regularizer=regularizers.l2(10**(-6)), activation=tf.nn.relu),
        layers.Dropout(0.1),
        layers.Dense(64, kernel_regularizer=regularizers.l2(10**(-6)), activation=tf.nn.relu),
        layers.Dropout(0.1),
        layers.Dense(64, kernel_regularizer=regularizers.l2(10**(-6)), activation=tf.nn.relu),
...
if SaveModel:
    ...
    t_df.to_csv('df_training_histroy_simpleneuralnet_model'+'.csv', index=False)
    f_df.to_csv('df_crossvalidation_simpleneuralnet_model'+'.csv', index=False)
...
if get_missclass:
    ...
    df.to_csv(thumbnail_dir+'missclassifications_simple_NN'+'.csv', index=False)
